...
export default function App(){
 ...
 const [question,setQuestion]=useState("Five outlets with lowest case fill rate last month"),[answer,setAnswer]=useState<any>(),[asking,setAsking]=useState(false),[askError,setAskError]=useState("");
 ...
 const ask=async(e:React.FormEvent)=>{e.preventDefault();if(!question.trim())return;setAsking(true);setAskError("");try{const r=await fetch("/api/chat",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({question,region})});const body=await r.json().catch(()=>null);if(!r.ok)throw Error(body?.error??`Ask Kestrel returned ${r.status}`);setAnswer(body)}catch(e){setAnswer(undefined);setAskError(e instanceof Error?e.message:"Unable to answer this question")}finally{setAsking(false)}};
 ...
 return <div className="app">
  ...
  <main>
   ...
   <section className="panel ask"><div className="ask-copy"><span className="eyebrow">AI-ASSISTED ANALYTICS</span><h2>Ask Kestrel</h2><p>Ask any question in plain English. Kestrel uses reviewed dashboard metrics when they are relevant.</p><div className="chips">{["OTIF by region","Categories driving return value","Temperature excursions per 100 chilled deliveries","Routes >2 hours late"].map(q=><button key={q} onClick={()=>setQuestion(q)}>{q}</button>)}</div></div><div className="ask-box"><form onSubmit={ask}><input value={question} onChange={e=>setQuestion(e.target.value)} aria-label="Ask Kestrel a question" placeholder="Ask a question…" maxLength={2000}/><button disabled={asking||!question.trim()}>{asking?"Thinking…":"Ask →"}</button></form>{askError&&<p className="ask-error" role="alert">{askError}</p>}{answer&&<div className="answer"><b>{answer.title}</b><p>{answer.explanation}</p>{answer.rows?.length>0&&<div className="answer-rows">{answer.rows.slice(0,5).map((x:Row,i:number)=><div key={i}><span>{String(x.name??x.month??x.sku_code??`Result ${i+1}`)}</span><b>{x.case_fill!=null?pct(Number(x.case_fill)):x.otif!=null?pct(Number(x.otif)):x.value!=null?inr(Number(x.value)):x.excursions_per_100!=null?String(x.excursions_per_100):x.cases!=null?number(Number(x.cases)):""}</b></div>)}</div>}</div>}</div></section>
